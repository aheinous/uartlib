...
SCENARIO("BYTE BUFF USAGE"){
    GIVEN("just initted"){
        ...
        WHEN("full"){
            for( uint8_t i=0; i< EZP_BYTE_BUFFER_CAPACITY-1; i++){
            ...
            THEN("acts full"){
                ...
                REQUIRE_FALSE(byteBuff_isEmpty(&bbuff));
                REQUIRE(byteBuff_size(&bbuff) == EZP_BYTE_BUFFER_CAPACITY-1);
            }
            THEN("peeks work"){
                for( uint8_t i=0; i< EZP_BYTE_BUFFER_CAPACITY-1; i++){
            ...
            THEN("can pop wrap around"){
                ...
                REQUIRE(byteBuff_size(&bbuff) == EZP_BYTE_BUFFER_CAPACITY-1-16);
                for( uint8_t i=EZP_BYTE_BUFFER_CAPACITY-1; i<EZP_BYTE_BUFFER_CAPACITY-1+16; i++){
                    REQUIRE(byteBuff_push(&bbuff, 2*i) == EZP_OK);
                }
                for(uint8_t i=0; i<EZP_BYTE_BUFFER_CAPACITY-1; i++){
                    REQUIRE(byteBuff_peek(&bbuff, i, &b) == EZP_OK);
